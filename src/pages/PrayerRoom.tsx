    ...
    <div className="min-h-screen bg-black">
      ...
      >
        ...
        <div className="max-w-3xl mx-auto relative z-10">
          <header className="text-center mb-12 bg-[#1e1e1e]/60 p-6 rounded-2xl backdrop-blur-sm border border-[#333]">
            <h1 className="font-serif text-2xl md:text-3xl mb-4 text-[#fef3c7]">"너는 기도할 때에 네 골방에 들어가 문을 닫고 은밀한 중에 계신 네 아버지께 기도하라..."</h1>
            <p className="text-[#f59e0b] font-serif text-2xl">- 마태복음 6:6 -</p>
          </header>

        <form onSubmit={handleSubmit} className="bg-[#1e1e1e]/80 backdrop-blur-sm p-6 rounded-2xl mb-12 shadow-lg border border-[#333]">
          <textarea
            value={content}
            onChange={(e) => setContent(e.target.value)}
            placeholder="지존자의 은밀한 곳에 머무는 시간입니다 (시편 91:1)"
            className="w-full bg-transparent border-b border-[#444] p-2 mb-4 focus:outline-none focus:border-[#f59e0b] transition text-[#fef3c7] placeholder-[#666]"
            rows={3}
          />
          <div className="flex items-center justify-between">
            ...
            <button type="submit" className="bg-[#f59e0b] text-black px-6 py-2 rounded-full font-bold hover:bg-[#d97706] transition">기도 올리기</button>
          </div>
        </form>

        <div className="space-y-6 overflow-y-auto max-h-[calc(100vh-300px)] pr-2 no-scrollbar">
          {requests.map(req => (
            <motion.div key={req.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-[#2d2a1a]/80 backdrop-blur-sm p-6 rounded-2xl border border-[#f59e0b]/30 shadow-lg">
              <div className="flex justify-between items-start mb-4">
                <p className="font-bold text-[#fef3c7]">{req.authorName}</p>
                <div className="flex items-center gap-4">
                  {req.isPrivate && <Lock size={20} className="text-[#f59e0b]" />}
                  {(role === 'admin' || req.uid === user?.uid) && (
                    <>
                      <button onClick={() => { setEditingId(req.id); setEditContent(req.content); }} className="text-[#fef3c7] hover:text-[#f59e0b] p-1"><Edit2 size={24} /></button>
                      <button onClick={() => handleDelete(req.id)} className="text-red-400 hover:text-red-300 p-1"><Trash2 size={24} /></button>
                    </>
                  )}
                </div>
              </div>
              {editingId === req.id ? (
                <div className="mb-6">
                  ...
                  <button onClick={() => handleUpdate(req.id)} className="mt-2 bg-[#f59e0b] text-black px-4 py-1 rounded font-bold">저장</button>
                </div>
              ) : <p className="mb-6 whitespace-pre-wrap text-[#fef3c7]">{req.content}</p>}
              <button onClick={() => handlePray(req.id, req.prayedBy || [])} className="flex items-center gap-2 text-[#f59e0b] hover:text-[#d97706] transition">
                <Heart size={24} />
                <span className="font-bold text-lg">함께 기도합니다 ({req.prayCount})</span>
              </button>
            </motion.div>
          ))}
        </div>
