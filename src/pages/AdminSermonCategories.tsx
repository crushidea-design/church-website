...
export default function AdminSermonCategories() {
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, 'sermon_categories'), {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'sermon_categories', deletingId));
      setDeletingId(null);
      setDeletingName('');
    } catch (error) {
  ...
  const moveCategory = async (index: number, direction: 'up' | 'down') => {
    ...
    newCategories.forEach((cat, idx) => {
      batch.update(doc(db, 'sermon_categories', cat.id), { order: idx });
    });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="p-3 bg-white rounded-2xl shadow-sm border border-wood-200">
            <Video className="text-wood-900" size={24} />
          </div>
          <div>
            <h1 className="text-3xl font-serif font-bold text-wood-900">말씀 서재 카테고리 관리</h1>
            <p className="text-wood-600">설교 영상들을 묶을 재생목록(카테고리)을 관리합니다.</p>
          </div>
        </div>
