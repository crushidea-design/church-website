...
- 아이·학부모·일반 이용자에게는 가장 최근 주일의 자료와 말씀 열매가 기본으로 보임
- 교사·관리자는 **다가오는 주일 준비** 버튼으로 다음 주일 자료와 말씀 열매 초안을 미리 확인
- 주일 당일이 되면 기본 화면이 새 주일 자료와 새 말씀 열매로 전환
- 학생들이 매일 1회 “오늘 실천했어요” 체크
- 학부모는 자녀 체크 시마다 알림 수신
- 교사는 자기 패널에서 담당 반 진행 상황 모니터링 (수요일 이후 미시작 학생은 빨간 강조로 표시됨)
- 새 학부모/교사 가입자가 생기면 **다음세대 관리 → 말씀 열매** 탭에서 연결·반 배정
...
회원·반·연결 같은 “주차마다 바뀌지 않는” 설정은 **다음세대 관리 페이지**의 **말씀 열매** 탭에 모여 있습니다.

진입 경로:
- 다음세대 페이지 헤더 → ⚙️(관리) 버튼
- 상단 탭에서 **말씀 열매** 클릭

이 탭 안에 다음 세 섹션이 순서대로 있습니다:
1. 반(그룹) 관리
2. 부모-자녀 연결
3. 데이터 마이그레이션

> 주차별 작업(이번 주 강의·카드·학생 작은 순종·게시 알림)은 **다음세대 → 유초등부 → 이번 주 말씀 열매 → 관리** 패널에서 진행합니다. 두 화면이 분리돼 있는 점만 기억하면 됩니다.

### 3.4 반(그룹) 만들기

다음세대 관리 → **말씀 열매** 탭 → **반(그룹) 관리** 섹션에서:
...
1. 학부모 회원이 가입 후 승인되면 학생 체크박스로 자녀 연결
2. 저장하면 그 학부모만 자녀 진행을 볼 수 있고, 자녀가 체크할 때마다 푸시 수신

### 3.6 그룹 데이터 백필 (그룹 도입 직후 한 번만)

같은 탭의 **데이터 마이그레이션** 섹션에서:

1. **그룹 백필 실행** 클릭
2. 결과 보고서에서 “백필 적용” 숫자 확인
...
4. 그래도 모르면 **이번 주 말씀 열매 → 관리 → 강의원고로 카드 생성** 버튼으로 직접 만들고 게시 (텍스트 붙여넣기 또는 PDF 불러오기 모두 지원)

### 6.2 “학부모가 자녀 진행을 못 봐요”

- **다음세대 관리 → 말씀 열매 → 부모-자녀 연결** 섹션에서 해당 학부모 카드의 자녀 체크박스가 켜져 있는지 확인
- 학부모가 “학부모” 부서로 가입했고 `member` 상태인지 확인 (관리자 회원 승인)

### 6.3 “교사가 다른 반 학생을 봐요”

- **다음세대 관리 → 말씀 열매 → 반(그룹) 관리** 섹션에서 그 교사의 담당 반(groupIds) 확인
- 담당 반이 **비어있으면** 임시로 모두 보입니다 (운영 편의용)
- 정식 운영하려면 모든 교사에게 담당 반을 명시 지정

### 6.4 “학생이 ‘작은 순종이 등록되지 않았어요’ 메시지를 봐요”

- 해당 주차 fruit 도큐먼트에 그 학생의 progress가 아직 없습니다
- **이번 주 말씀 열매 → 관리 → 아이별 작은 순종** 표에서 그 학생의 실천 입력 후 저장
...
- 매일 23:00 자동 갱신
- 즉시 갱신하려면 **이번 주 말씀 열매 → 관리 → 공동체 현황 게시** 버튼 클릭
...
├── pdfText.ts              # PDF 텍스트 추출 (관리 패널의 PDF 불러오기)
├── WordFruitPanel.tsx      # 사용자 패널 (학생/학부모/교사/비로그인)
├── WordFruitAdmin.tsx      # 주간 관리 (이번 주 강의·카드·실천·공동체·게시 알림)
├── WordFruitSettings.tsx   # 비주간 설정 (반 관리·부모-자녀 연결·마이그레이션)
